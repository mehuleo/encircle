var Game = new(function() {
    var gameStage = [{},stageOneLevels, stageTwoLevels, stageThreeLevels, stageFourLevels],
        stage = 0,
        level = 0,
        levelInfo,
        currentScore = 297,
    ...
    this.init = function(info, debugging) {
        ...
        levelInfo = info || gameStage[stage][level];
    ...
    this.handleNext = function(){
        ...
        var l = localStorage.getItem('level');
        if(!s || !l){ s=1; l=1; }
        if(l>=levelsInAStage){ s++; l=1; }
        else l++;
        if(s > totStage || l>levelsInAStage){ s=1; l=1; }
        localStorage.setItem('stage', s);
        localStorage.setItem('level', l);
        Game.handleTakeScreenshot(1);
        Game.init();
